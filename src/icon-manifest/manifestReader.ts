...
export class ManifestReader {
  public static iconsDisabled(name: string, isFile: boolean = true): boolean {
    ...
    const iconsJson =
      iconManifest && (parseJSON(iconManifest) as models.IIconSchema);
    const prefix = isFile
      ? extensionSettings.manifestFilePrefix
      : extensionSettings.manifestFolderPrefix;
    const suffix = Reflect.ownKeys(models.Projects).some(
  ...
  public static folderIconsDisabled(
    func: (iconsJson: models.IIconSchema) => boolean,
  ): boolean {
    const iconManifest = this.getIconManifest();
    const iconsJson =
      iconManifest && (parseJSON(iconManifest) as models.IIconSchema);
