...
export function mergeConfig(
  ...
  iconGenerator: models.IIconGenerator,
): models.IIconSchema {
  ...
  const folders: models.IFolderCollection = {
    default: mergeDefaultFolders(
      defaultCustomFolders,
      supportedFolders.default,
    ),
    supported: mergeSupported(
      supportedCustomFolders,
...
export function toggleHideFoldersPreset(
  ...
  const folderIcons = folders.supported
    .filter(x => !x.disabled)
    .map(x => x.icon);
  const collection = togglePreset<models.IFolderCollection>(
    disable,
    folderIcons,
    folders,
  );
