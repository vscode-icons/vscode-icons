...
export function manageAutoApplyCustomizations(
  ...
  userConfig: IVSIcons,
  applyCustomizationCommandFn: () => void,
): void {
  if (!isNewVersion) {
    return;
  }
...
export function manageApplyCustomizations(
  ...
  additionalTitles?: IVSCodeMessageItem[],
): void {
  if (
    !newConfig.dontShowConfigManuallyChangedMessage &&
    configChanged(utils.flatten(oldConfig), newConfig)
  ) {
...
function configChanged(prevConfig, currentConfig) {
  for (const key of Reflect.ownKeys(prevConfig) as string[]) {
    ...
    const cond1 =
      ...
      newValue.length !==
        _.intersectionWith(oldValue, newValue, _.isEqual).length;
