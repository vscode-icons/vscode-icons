...
import { FileFormat, IFileCollection, IIconSchema } from '../../src/models';
...
describe('IconGenerator: folders icon generation test', function() {
  context('ensures that', function() {
    ...
    context('default', function() {
      ...
      beforeEach(() => {
        schema = new IconGenerator(vscode, defaultSchema).generateJson(
          emptyFileCollection,
          folders,
        );
      });
      ...
      it('root folder has an open icon path', function() {
        expect(
          schema.iconDefinitions._root_folder_open.iconPath,
        ).not.to.be.equal('');
      });
          ...
          context('folder', function() {
            it('has an associated icon file', function() {
              folders.supported.forEach(folder => {
                const filename = `${settings.folderPrefix}${folder.icon}${
                  settings.iconSuffix
      ...
      context('each supported', function() {
        context('folder that has not a light theme version', function() {
          ...
          beforeEach(() => {
            ...
            schema = new IconGenerator(vscode, dSchema).generateJson(
              emptyFileCollection,
              folders,
            );
          });
