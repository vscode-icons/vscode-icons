...
describe('CleanUp: tests', function () {
  ...
  context('ensures that', function () {
          ...
          context('and extension\'s installed directory is', function () {
            ...
            it('\'.code-oss-dev\'',
              function () {
                ...
                const userPath = utils.pathUnixJoin(appPath, 'code-oss-dev', 'User');
                expect(process.platform).to.be.equal('free-bsd');
                expect(cleanUp.getAppUserPath(dirPath)).to.be.equal(userPath);
              });
        ...
        context('win32 (windows)', function () {
          ...
          context('and extension\'s installed directory is', function () {
            ...
            it('\'.code-oss-dev\'',
              function () {
                dirPath = dirPath.replace('%appDir%', '.vscode-oss-dev');
                const userPath = utils.pathUnixJoin(process.env.APPDATA, 'code-oss-dev', 'User');
                expect(process.platform).to.be.equal('win32');
                expect(cleanUp.getAppUserPath(dirPath)).to.be.equal(userPath);
              });
    ...
    context('function \'cleanUpVSIconsSettings\'', function () {
      ...
      it('log an Error message when deleting the file fails',
        function () {
    ...
    context('function \'cleanUpVSCodeSettings\'', function () {
      ...
      it('writes the \'vscode\' settings file',
        function () {
      ...
      it('logs an Error message when reading the file fails',
        function () {
          const consoleStub = sandbox.stub(console, 'error');
      ...
      it('logs an Error message when writting the file fails',
        function () {
          const consoleStub = sandbox.stub(console, 'error');
          const content = '{"vsicons.someProperty": true}';
          readFileStub.yields(null, content);
      ...
      it('does not do anything when read file is not JSON',
        function () {
      ...
      it('does not write the settings when there is no content',
        function () {
          const content = '';
          readFileStub.yields(null, content);
          cleanUp.cleanUpVSCodeSettings();
    ...
    context('function \'resetThemeSetting\'', function () {
      ...
      beforeEach(() => {
        const content = `{"${constants.vscode.iconThemeSetting}": "${constants.extensionName}"}`;
      ...
      it('to not reset the \'iconTheme\' setting, if it\'s not set to \'vscode-icons\'',
        function () {
          ...
          cleanUp.resetThemeSetting(settings);
          expect(settings[constants.vscode.iconThemeSetting]).to.be.equal('someOtherTheme');
        });
    ...
    context('function \'removeVSIconsSettings\'', function () {
      ...
      it('to remove only all \'vsicons\' settings',
        function () {
